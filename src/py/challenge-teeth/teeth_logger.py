    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if batch_idx % self.log_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    grid_X = torchvision.utils.make_grid(X[:, 0:num_images, 0:3, :, :])#Grab the first image, RGB channels only, X, Y. The time dimension is on dim=1
                    ...
                    grid_x = torchvision.utils.make_grid(x[:, 0:num_images, 0:1, :, :]/pl_module.out_channels)# The time dimension is on dim 1 grab only the first one
                    trainer.logger.experiment.add_image('x', grid_x, 0)

                    grid_y = torchvision.utils.make_grid(y[:, 0:num_images, :, :, :]/pl_module.out_channels)# The time dimension here is swapped after the permute and is on dim=2. It will grab the first image
