...
class TeethDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.surf_property:            
            ...
            # if any(item in surf_point_data_faces for item in self.t_wisdom): # if ID list contains any wisdom teeth ID
            #     weight = 5
            unique = torch.unique (surf_point_data_faces)
            if -1 in unique:
...
class TeethDataModule(pl.LightningDataModule):
    ...
    def pad_verts_faces(self, batch):
        ...
        return verts, faces, torch.cat(verts_data_faces), color_normals
