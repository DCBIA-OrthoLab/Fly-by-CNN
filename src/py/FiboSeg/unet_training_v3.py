...
####
"""
V3: UNETR  & Early Stopping, multiple loops with camera rotations in training
"""
####
####
...
class FlyByDataset(Dataset):
    ...
    def __getitem__(self, idx):        
        ...
        region_id = ToTensor(dtype=torch.int64, device=device)(vtk_to_numpy(surf_point_data))
...
def pad_verts_faces(batch):
    ...
    print(f'region_ids[0].shape {region_id[0].shape}')
    print(f'pad_seq_rid.shape {pad_seq_rid.shape}')

    print(f'torch.cat(region_ids_faces.shape) {torch.cat(region_ids_faces).shape}')
...
def main(): 
    ...
    """
    df_train.to_csv('train_data_1.csv',index=False)
    df_val.to_csv('val_data_1.csv',index = False)
    """
    ...
    train_dataloader = DataLoader(train_data, batch_size=batch_size, shuffle=True, collate_fn=pad_verts_faces, num_workers=10,pin_memory=True, prefetch_factor=4)
    val_dataloader = DataLoader(val_data, batch_size=batch_size, shuffle=True, collate_fn=pad_verts_faces, num_workers=10,pin_memory=True, prefetch_factor=4)
    """
    ...
    #model.load_state_dict(torch.load("best_metric_model_segmentation2d_array_v3.pth"))
        ...
        if (epoch) % val_interval == 0: # every two epochs : validation
            ...
            with torch.no_grad():
                ...
                if metric > best_metric:
                    ...
                    torch.save(model.state_dict(), "model_segmentation_02_02_unet_csv1.pth")
