...
def main():
    ...
    noise_lvl = 0.015
    dropout_lvl = 0.2
    model_fn = f"checkpoints/{NOW}_BINS_icolvl{ico_lvl}_noise{noise_lvl}_dropout{dropout_lvl}_seed{SEED}.pt"
    ...
    load_model = True
    model_to_load = "checkpoints/11_07_14h27_BINS_early_stop_MSE_with_test_split_res224_train_shuffle_icolvl1_noise0.01_dropout0.1_seed1651143.pt"
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():  # VALIDATION LOOP
            ...
            for batch, (vertex_features, face_features, age_at_birth, age_bin, weights) in enumerate(val_dataloader):
                ...
                mse_loss = weighted_mse_loss(x, Y,weights) 
                val_MAE += torch.mean(torch.abs(x-Y)).item()
            ...
            print(f'val loss (weighted MSE): {val_loss}')
