...
# def merge_jsonfile(filename):
#     if True in ['P'+ str(ocl) + '_' in filename for ocl in list(range(1,lenght))]
#     result = list()
#     for f1 in filename:
#         with open(f1, 'r') as infile:
#             result.append(json.load(infile))

#     datapath = os.path.normpath("/".join([args.dir_data, "data2"]))
#     if not os.path.exists(datapath):
#         os.makedirs(datapath)
#     with open(os.path.normpath("/".join([datapath, os.path.basename(f1).split("_")[0]+".json"])), 'w') as output_file:
...
if __name__ == '__main__':
    ...
    input_param.add_argument('--dir_data', type=str, help='Input directory with 3D images', default='/Users/luciacev-admin/Desktop/data')
    input_param.add_argument('--landmarks_dir', type=str, help='landmarks directory', default=parser.parse_args().dir_data+'/landmarks_RC')
    ...
    output_params.add_argument('--out', type=str, help='Output directory', default=parser.parse_args().dir_data+'/out')
