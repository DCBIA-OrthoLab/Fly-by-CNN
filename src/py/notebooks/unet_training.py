    ...
    if (epoch) % val_interval == 0: # every two epochs : validation
        ...
        with torch.no_grad():
            ...
            for batch, (V, F, Y, YF, F0, CN, FL,N) in enumerate(val_dataloader):   
                ...
                print("shape val_output : ",val_outputs.shape)
                print("shape val_images : ",val_images.shape)
            ...
            imgs_normals = val_images[:,0,:,:]
            imgs_normals = imgs_normals.unsqueeze(1)
            ...
            norm_rgb = torch.cat((imgs_normals,imgs_normals,imgs_normals),dim=1)
