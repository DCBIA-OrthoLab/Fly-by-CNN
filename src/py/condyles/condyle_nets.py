...
class CondyleClassification(pl.LightningModule):
    def __init__(self, class_weights=None, **kwargs):

        super(CondyleClassification, self).__init__()
        self.save_hyperparameters()
        ...
        cameras = FoVPerspectiveCameras(znear=0.01)
        raster_settings = RasterizationSettings(
...
class CondyleClassificationViT(pl.LightningModule):
    def __init__(self, class_weights=None, **kwargs):

        super(CondyleClassificationViT, self).__init__()
        self.save_hyperparameters()
        ...
        # feat.classifier = nn.Identity()
        # num_feat = 1280
        feat = torch.nn.Sequential(monai.networks.nets.ViT(in_channels=4, img_size=(256,256), patch_size=(16,16), pos_embed='conv', classification=False, spatial_dims=2),  ViTFeat())        

        self.F = TimeDistributed(feat)
        # self.V = nn.Sequential(nn.Linear(768, 128), nn.Tanh())
        self.V = nn.Linear(768, 256)
        self.A = SelfAttention(in_units=768, out_units=128)
        self.P = nn.Linear(256, self.hparams.out_classes)        
        ...
        if self.hparams.radius is not None and self.hparams.subdivision_level is not None:
            self.ico_sphere(radius=self.hparams.radius, subdivision_level=self.hparams.subdivision_level)
...
class CondyleClassificationDense(pl.LightningModule):
    def __init__(self, class_weights=None, **kwargs):

        super(CondyleClassificationDense, self).__init__()
        self.save_hyperparameters()
        ...
        # feat.classifier = nn.Identity()
        # num_feat = 1280
        feat = monai.networks.nets.DenseNet121(pretrained=True, spatial_dims=2, in_channels=4, out_channels=512)
    ...
    def render(self, V, F, CN):
        ...
        for camera_position in self.ico_verts:
            ...
            if self.hparams.norm_zbuf:
                zbuf_min = torch.min(zbuf)
                zbuf_max = torch.max(zbuf)
                zbuf = (zbuf - zbuf_min) / (zbuf_max - zbuf_min)
