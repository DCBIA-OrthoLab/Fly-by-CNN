...
def main(args):  
    ...
    for epoch in range(args.num_epoch):
        training( epoch, move_net, train_dataloader, phong_renderer, loss_function, optimizer, epoch_loss, writer)

        if (epoch +1 ) % test_interval == 0:
            print(epoch +1)
            validation(epoch,move_net,test_dataloader,phong_renderer,loss_function,list_distance,best_deplacment,best_deplacment_epoch,args.out)
...
if __name__ == '__main__':
    ...
    input_param.add_argument('--blur_radius',type=int, help='dlur raius', default=0)
